...
def home():
    return jsonify({
        "status": "online",
        "message": "Emote Bot API is running!",
        "endpoints": {
            "/join": "GET - Trigger emote with params: tc, uid1-uid6, emote_id",
...
def health():
    ...
    return jsonify({
        "status": "healthy",
        "bot_connected": online_writer is not None and whisper_writer is not None,
...
def join_team():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "emote_id must be integer"}), 400
    ...
    if loop is None:
        return jsonify({"status": "error", "message": "Bot not fully initialized yet"}), 503
...
def run_flask():
    # CRITICAL FIX: Bind to 0.0.0.0 and use PORT env var for Render deployment
...
async def MaiiiinE():
    ...
    # BOT LOGIN UID
    BOT_UID = int('15191694174')

    Uid, Pw = '4666672671', '4C8CBAC9CF4CCCD7EA6EAA506C41ECA6DE59A452D3EC6E093856F348BA05F5F0'
    ...
    flask_thread.start()
    print("Flask server started on separate thread")
...
if __name__ == '__main__':
    # This is the correct way to run the async main with Flask
